...
8. 遵守 `USER_VALIDATION_POLICY.md`：不得主动创建或执行真实用户验证；只有负责人明确要求本项目启用时才可进行。
...
- 工程测试不能因真实用户验证未启用而降低；未进行的用户验证不得写成通过或完成。
...
- 真实用户验证状态或范围变化必须更新 `USER_VALIDATION_POLICY.md` 和产品证据边界。
